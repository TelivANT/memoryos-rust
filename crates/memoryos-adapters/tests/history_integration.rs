...
use memoryos_core::{MemoryHistoryEntry, HistoryEventType};
...
async fn test_history_storage() {
    ...
    // 添加条目
    storage.add_entry(entry.clone()).await.expect("Failed to add entry");
    
    // 查询历史
    let history = storage.get_history("test_memory_123").await.expect("Failed to get history");
