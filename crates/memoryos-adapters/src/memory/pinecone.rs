...
impl PineconeStorage {
    ...
    fn get_index_url(&self, index_name: &str) -> String {
        format!(
            ...
            index_name, self.environment, self.environment
        )
    }
}
...
impl VectorStorage for PineconeStorage {
    ...
    async fn add_short_term_message(
        ...
        let url = format!(
            "{}/vectors/upsert",
            self.get_index_url(&self.shortterm_index)
        );
        self.client
            .post(&url)
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Pinecone upsert short-term failed: {}", e))
            })?;

        debug!(
            "Stored short-term message in Pinecone for user: {}",
            user_id
        );
        Ok(())
    }
    ...
    async fn get_short_term_messages(
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Pinecone query short-term failed: {}", e))
            })?
            .json::<PineconeQueryResponse>()
            .await
            .map_err(|e| AppError::ExternalService(format!("Pinecone parse failed: {}", e)))?;

        let mut messages: Vec<memoryos_core::Message> = response
            .matches
            .into_iter()
        ...
        debug!(
            "Retrieved {} short-term messages from Pinecone",
            messages.len()
        );
        Ok(messages)
    }
    ...
    async fn clear_short_term(&self, user_id: &str) -> Result<(), AppError> {
        ...
        self.client
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Pinecone delete request failed: {}", e))
            })?
            .error_for_status()
    ...
    async fn search_segments(
        ...
        let segments: Vec<MidTermSegment> = response
            .matches
            .into_iter()
            .filter_map(|m| {
    ...
    async fn store_long_term(&self, memory: LongTermMemory) -> Result<(), AppError> {
        ...
        let url = format!(
            "{}/vectors/upsert",
            self.get_index_url(&self.longterm_index)
        );
        self.client
            .post(&url)
            .header("Api-Key", &self.api_key)
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Pinecone store long-term failed: {}", e))
            })?;

        debug!(
            "Stored long-term memory in Pinecone for user: {}",
            memory.user_id
        );
        Ok(())
    }
