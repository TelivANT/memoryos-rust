...
use opendal::{Operator, Builder, Scheme};
    ...
    pub fn new(scheme: &str, map: HashMap<String, String>) -> Result<Self, AppError> {
        let op = match scheme {
            "s3" | "minio" => {
                ...
                if let Some(root) = map.get("root") { builder.root(root); }
                if let Some(bucket) = map.get("bucket") { builder.bucket(bucket); }
                if let Some(endpoint) = map.get("endpoint") { builder.endpoint(endpoint); }
                if let Some(ak) = map.get("access_key_id") { builder.access_key_id(ak); }
                if let Some(sk) = map.get("secret_access_key") { builder.secret_access_key(sk); }
                if let Some(region) = map.get("region") { builder.region(region); }
                Operator::new(builder).map_err(|e| AppError::Config(e.to_string()))?.finish()
            },
            "fs" => {
            ...
            _ => return Err(AppError::Config(format!("Unsupported storage scheme: {}", scheme))),
        };
