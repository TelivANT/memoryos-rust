...
impl OptimizedRetriever {
    ...
    pub async fn retrieve_with_cache<F, Fut>(&self, query: &str, embed_fn: F) -> Result<Vec<f32>>
    where
        F: Fn(String) -> Fut,
...
mod tests {
    ...
    async fn test_optimized_retriever() {
        ...
        let embedding1 = retriever
            .retrieve_with_cache("test", |_| async { Ok(vec![1.0, 2.0, 3.0]) })
            .await
            .unwrap();

        // Second call should hit cache
        let embedding2 = retriever
            .retrieve_with_cache("test", |_| async {
