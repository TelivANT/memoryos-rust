...
impl HeatTracker {
    ...
    pub fn get_stats(&self, segments: &[MidTermSegment]) -> HeatStats {
        ...
        let qa_count = segments
            .iter()
            .filter(|s| s.memory_type == MemoryType::QA)
            .count();
        let candidate_count = segments
            .iter()
            .filter(|s| s.memory_type == MemoryType::FaqCandidate)
            .count();
        let faq_count = segments
            .iter()
            .filter(|s| s.memory_type == MemoryType::Faq)
            .count();
