...
use memoryos_core::{
    llm::{RouterContext, RouteTier, RouteDecision},
    security::ComplianceResult,
    AppError,
};
use memoryos_ports::{ChatRequest, ChatResponse, ChatMessage};
...
pub async fn chat_completions(
    ...
    let router_ctx = RouterContext {
        ...
        is_faq_match: false,
        has_sensitive_keywords: matches!(state.shield.check_compliance(&last_msg), ComplianceResult::RequiresLocal),
    };
    ...
    if let RouteDecision { tier: RouteTier::DirectHit, direct_response: Some(content), .. } = &decision {
    ...
    let adapter = state.get_adapter(target_provider)
