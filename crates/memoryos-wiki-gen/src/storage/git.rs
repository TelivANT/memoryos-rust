...
pub enum GitAuth {
    None,
    Token(String),
    SshKey { private_key: PathBuf, passphrase: Option<String> },
}
...
impl GitConnector {
    ...
    pub fn with_ssh_key(mut self, private_key: PathBuf, passphrase: Option<String>) -> Self {
        self.auth = GitAuth::SshKey { private_key, passphrase };
        self
    }
}
...
impl StorageConnector for GitConnector {
    async fn connect(&mut self) -> Result<()> {
        ...
        let auth = match &self.auth {
            ...
            GitAuth::SshKey { private_key, passphrase } => {
                GitAuth::SshKey { 
        ...
        let repo_path = tokio::task::spawn_blocking(move || -> Result<PathBuf> {
            ...
            Ok(temp_dir)
        }).await.map_err(|e| WikiGenError::Storage(format!("Task join error: {}", e)))??;
